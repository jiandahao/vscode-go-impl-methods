...
export class InterfaceStubsGenerator {
  ...
  receiver(): Receiver | undefined {
    ...
    return { name: matches[1], type_: matches[2], kind: 'iface',range: line?.range };
  }
  ...
  implement(interface_: string, receiver: Receiver, callback?: (stubs: string) => void): void {
    vscode.window.withProgress({
      ...
      return new Promise((resolve) => {
        const impl = cp.exec(`impler "${receiver?.name} ${receiver?.type_}" ${receiver?.kind} ${interface_}`,
...
export class InterfaceExtractor {
  ...
  constructor(private editor: vscode.TextEditor | undefined) {
    this.pattern =  /^\/\/\s*([a-zA-Z_][a-zA-Z0-9_]*)$/;
  }
  ...
  receiver(): Receiver | undefined {
    ...
    return { name: matches[1], type_: "", kind:'struct',range: line?.range };
  }
  ...
  extract(structure_: string, receiver: Receiver,callback?: (stubs: string) => void):void {
    vscode.window.withProgress({
      location: vscode.ProgressLocation.Notification,
      title: "Extracting interface..."
    }, (progress, token) => {
      return new Promise((resolve) => {
        const impl = cp.exec(`impler "${receiver?.name}" ${receiver?.kind} ${structure_}`,
        { cwd: dirname((this.editor as vscode.TextEditor).document.fileName) },
        (error, stdout, stderr) => {
          if (error) {
            vscode.window.showInformationMessage(stderr);
